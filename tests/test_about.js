describe('About page loads', function () {
    var h1_tags = element.all(by.tagName('h1'));
    var active_class = element.all(by.css('.active'));
    var project_issues_link = element.all(by.css('div.col-md-9.ng-scope a')).last(); // this locator is brittle and should be changed
    beforeEach(function () {
    ...
    it('should have the correct text for the h1', function () {
        expect(h1_tags.first().getText()).toEqual('About this service');
    });
    it('should have the correct menu item set as active', function () {
        expect(active_class.count()).toEqual(1);
        expect(active_class.first().getText()).toEqual('About this service');
    });
    it('should have a link to project issues', function () {
        expect(project_issues_link.getText()).toEqual('the project issues page');
